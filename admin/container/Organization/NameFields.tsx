...
const GENERATE_SLUG = gql`
  query generateOrganizationSlug($name: String!) {
    generateOrganizationSlug(name: $name) {
      slug
    }
  }
`;

export class OrganizationNameFields extends Component<Props, {}> {
  handleBlur = async (client, value) => {
    const result = await client.query({
      query: GENERATE_SLUG,
      variables: { name: value },
    });
    const slug = result.data.generateOrganizationSlug.slug;
