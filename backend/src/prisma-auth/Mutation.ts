...
export const auth = {
  ...
  async inviteUser(
    ...
    function createEmployment(userId: string) {
    ...
    if (existingUser) {
      const hasRestaurant = existingUser.employments!.some(
        e => e.restaurant.id === data.restaurantId
      );
      if (hasRestaurant) {
        throw new Error('User with email already exists in this restaurant');
      }

      await createEmployment(existingUser.id);
    ...
    await createEmployment(newUser.id);
